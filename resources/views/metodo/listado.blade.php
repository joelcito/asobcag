    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header" id="kt_modal_add_user_header">
                <h3 class="fw-bold">FORMULARIO DE METODO <span class="text-info" id="nombre_busqueda"></span></h3>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body scroll-y">
                <form id="formularioMetodo">
            ...
            <div class="modal-footer">
                <div class="row">
                    <div class="col-md-12">
                        <button class="btn btn-sm w-100 btn-success" onclick="guardarMetodo()">Guardar</button>
                    </div>
                </div>
            </div>
            <!--end::Modal body-->
            ...
            <div class="card">
                <div class="card-header flex-wrap bg-light-info py-4">
                    <div id="kt_app_toolbar_container" class="app-container container-xxlg d-flex flex-stack">
                        ...
                        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                            <!--begin::Title-->
                            <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">LISTADO DE METODOS</h1>
                            <!--end::Title-->
                        </div>
                        <!--end::Page title-->
                        ...
                        <div class="d-flex gap-2 gap-lg-3">
                            <a class="btn btn-sm fw-bold btn-primary" onclick="modalNuevoMetodo()"><i class="fa fa-plus"></i>Nuevo Metodo</a>
                        </div>
...
@section('js')
    ...
    <script>
        ...
        function guardarMetodo(){
            let datos = $('#formularioMetodo').serializeArray();
            $.ajax({
                url: "{{ route('metodo.guardarMetodo') }}",
                method: "POST",
                data: datos,
                success: function (resultado) {
                    if(resultado.estado){
        ...
        function editarMetodo(metodo){
            ...
            Object.keys(metodo).forEach(key => {
                let input = $(`#${key}`);
                if (input.length) {
                    input.val(metodo[key]);
                }
            });
            $('#modalMetodo').modal('show')
        }

        function eliminarMetodo(metodo){
            Swal.fire({
                title: "Quieres eliminar "+metodo.nombre,
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ route('metodo.eliminarMetodo') }}",
                        method: "POST",
                        data: metodo,
                        success: function (resultado) {
                            if(resultado.estado){
